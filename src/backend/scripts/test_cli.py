...
def cmd_chat(
    ...
    try:
        ...
        print(f"{YELLOW}Make sure the server is running: uvicorn main:app --reload --port 8000{RESET}")
        return None
    except Exception as e:
        print(f"{RED}Error: {e}{RESET}")
        return None
...
def main():
    ...
    if not check_server():
        ...
        print(f"{DIM}Start it with: uvicorn main:app --reload --port 8000{RESET}\n")
